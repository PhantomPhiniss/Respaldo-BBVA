...
#######################################################################################
# BBVA Mexico#
# Autor          : Diego Rodrigo Fern�ndez Zamora#
# Proposito      : Shell para la ejecuci�n del Des ofuscamiento                      #
####################################################################################### 
...
PASS_PROD=
INPUT_FILE=ARCH_INV_AUTOIND_AAAAMMDD.txt
OUTPUT_FILE=ARCH_RES_AUTOIND_AAAAMMDD.txt
...
main_descofusca ()
{
   ...
   if [ -e ${OUTPUT_PATH}/${OUTPUT_FILE} ]
      then
         ...
         echo "Finaliza ejecuci�n DesOfuscamiento ${PROCESO}"
      exit 0
   else 
